...
for i = 1:length(resolutions)
    ...
    y_zcurve = (m / r) * ((m^2 / r) * (1 + 1 / sqrt(2)) + ((m^2 / r^2) - 1) * r / 2) + m - r / 2;

    coordinates(i, :) = [ y_scan, y_Dscan, y_circle, y_LH, y_triangular, y_zcurve];
end
...
legend('Scan', 'Double scan', 'Circles', 'Linear Hexagonal', 'Triangular scan', 'Z-Curve', 'Location', 'Best');
colormap('parula');

% Display values of all traversal path lengths
disp('Resolutions  Scan    Double Scan Circles Linear Hexagonal Triangular Z-Curve');
